...
async function simulationSequence() {
    ...
    for (const name of agentNames) {
        updateAgentCardUI(name as AgentName, 'THINKING', 'Analyzing prompt...');
    }
    await sleep(1500);
...
async function runMultiAgentConsensus() {
    ...
    // Wait for panel to be visible to get correct positions
    await sleep(50);
    
    startVisualization();
    await simulationSequence();
...
function handleLoadPersonaSet() {
    ...
    currentConfiguration.agents.forEach((agent, index) => {
        if (personaSet.personas[agent.name]) {
            currentConfiguration.agents[index].persona = personaSet.personas[agent.name];
        }
    });
    
    renderPersonaEditor();
    personaSetNameInput.value = selectedName;
    alert(`Persona set "${selectedName}" loaded and applied.`);
}
...
function init() {
    ...
    flowCtx = flowCanvas.getContext('2d') as CanvasRenderingContext2D;
    ...
    orchestrateButton.addEventListener('click', runMultiAgentConsensus);
    ...
    currentConfiguration = JSON.parse(JSON.stringify(savedConfigurations["Default Orchestration"] || Object.values(savedConfigurations)[0]));
    populateConfigSelector();
    populatePersonaSetSelector();
    renderPersonaEditor();
    initializeAgents();
